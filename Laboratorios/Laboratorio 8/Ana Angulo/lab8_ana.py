...
delay_rsp = int(1 * fs_rsp)   # 1 segundo
delay_acc = int(2 * fs_acc)   # 2 segundos
...
# Respiración -> 500 Hz
rsp_resampled = resample(rsp_delayed, len(ecg))

# Acelerómetro -> 500 Hz
acc_resampled = resample(acc_delayed, len(ecg))
...
plt.subplot(3,1,2)
plt.plot(t_ecg, rsp_sync)
plt.title('Respiración Resampleada y Sincronizada')
plt.xlabel('Tiempo (s)')
plt.ylabel('Amplitud')
plt.grid(True)

# Acelerómetro sincronizado
plt.subplot(3,1,3)
plt.plot(t_ecg, acc_sync)
plt.title('Acelerómetro Resampleado y Sincronizado')
plt.xlabel('Tiempo (s)')
plt.ylabel('Amplitud')
plt.grid(True)
